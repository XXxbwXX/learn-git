...
`--hard` 手滑了？在 reflog 里找到出错前的位置，`git reset --hard e4f5g6h` 整个回来。**只要提交过的东西，短期内基本都找得回来**——reflog 条目默认保留 90 天（不可达的 30 天），日常抢救完全够用。

## 四、选择指南

| 情况 | 用药 |
|---|---|
| 提交信息写错 / 漏文件，未推送 | `commit --amend`（第二章第 03 课） |
